...
** to the Versatile AB926 serial port A.  Otherwise, I/O is targeted
** to the debugger console.  
** 
** Defining USE_TARGET_CLOCK targets the clocking mechanism to the
** Versatile AB926EJ-S reference clock.  Otherwise, clocking is 
...
#ifdef LIGHT_LED
#include "versatile.h"
#define SYS_LED ((volatile unsigned int *)(SYSBase + 0x08))
#endif
...
** the Versatile AB926 UART.  __backspace() must also be retargeted
...
int fputc(int ch, FILE *f)
{
    ...
    #ifdef LIGHT_LED
    // Binary counter of every 50 characters will be output to the 8 LEDs
    static char ch_cnt = 50;
    ch_cnt--;
    if (ch_cnt == 0){
       *SYS_LED = (*SYS_LED+1)&0xff;
       ch_cnt = 50;
    }
    #endif
...
int fgetc(FILE *f)
{
    ...
    #ifdef LIGHT_LED
    // Binary counter of every input character will be output to the 8 LEDs
    *SYS_LED = (*SYS_LED+1)&0xff;
    #endif
